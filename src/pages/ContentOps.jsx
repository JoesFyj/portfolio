...
export async function runFullPipeline(topicTitle, options = {}) {
  ...
  try {
    ...
    addLog(`【评审Agent】综合评分 ${review.score}/10 ${review.passed ? '✓ 通过' : '⚠ 需优化'}`, review.passed ? 'success' : 'warning')
    
    // 保存到飞书
    addLog('【发布Agent】正在保存到飞书文案库...', 'process')
    const saveResult = await saveToFeishu(title, content, {
      category: options.category,
      platforms: options.platforms || ['抖音'],
    })
    
    if (saveResult.record_id) {
      addLog(`【发布Agent】已保存到飞书 (ID: ${saveResult.record_id})`, 'success')
    } else {
      addLog('【发布Agent】飞书保存失败，可手动复制文案', 'warning')
    }
    
    // 格式化
    addLog('【发布Agent】正在生成多平台格式...', 'process')
    const formatted = await formatForPlatforms(title, content, options.platforms || ['wechat', 'douyin', 'xhs'])
    addLog('【发布Agent】多平台格式生成完成', 'success')
